...
// Fechar o menu ao clicar no corpo do texto
document.addEventListener("click", (event) => {
  const target = event.target;
  const isMenuOpen = body.classList.contains("menu-nav-active");

  if (isMenuOpen && !target.closest("#navbar") && !target.closest(".menu-mobile")) {
    body.classList.remove("menu-nav-active");
    menuMobile.classList.replace("bi-x", "bi-list");
  }
});
...
// Remover a mensagem de sucesso depois de 5 segundos
setTimeout(() => {
